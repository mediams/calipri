...
def build_et6_axis_matrix_report(
    ...
    def _parse_pair(value: str) -> list[float]:
        ...
        out: list[float] = []
        for p in parts:
            p_norm = p.replace(".", "").replace(",", ".") if "," in p else p
            try:
                out.append(float(p_norm))
            except ValueError:
                continue
        return out

    motor_axes = {"11", "12", "22", "21"}
    flagged_axes: set[str] = set()
    for axis, pair_value in axis_reference_map.items():
        values = _parse_pair(pair_value)
        if not values:
            continue
        if axis in motor_axes:
            if any(v < 920 or v > 990 for v in values):
                flagged_axes.add(axis)
        else:
            if any(v < 730 or v > 760 for v in values):
                flagged_axes.add(axis)
    ...
    def build_matrix_table(input_df):
        ...
        # Подчеркиваем заголовки осей, где Dlk вышел за допустимые пределы.
        for idx, header in enumerate(headers):
            if header.endswith(("L", "R")):
                axis = header[:-1]
                if axis in flagged_axes:
                    style_commands.append(("LINEBELOW", (idx, 0), (idx, 0), 2.2, colors.yellow))
