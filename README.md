...
- Параметры блоков `REDBOX` и `PZB` заполняются автоматически из данных CSV по соответствующим осям.
- После блока `REDBOX/PZB` добавлена отдельная таблица для поезда `Flirt 3 CHI (ENR)` по заданному шаблону.
